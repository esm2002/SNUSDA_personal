...
def control_choose(prediction): # weighted average for recent 5 frames
    ...
    if len(control) > 4:
        ...
        if mean_val < 2: # linear interpoloation : left_max = 15, middle = 0, right_max = -12
...
def Steer_publisher():
    ...
    model = tf.keras.models.load_model("/home/fmtc/catkin_ws/src/race/scripts/best_ss.h5") # Our CNN model load (trained with our data)
    rate = rospy.Rate(32)
